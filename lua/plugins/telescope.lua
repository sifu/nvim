...
local function _1_()
  ...
  telescope.setup({defaults = {file_ignore_patterns = {"node_modules"}, dynamic_preview_title = true, mappings = {i = {["<esc>"] = actions.close, ["<c-q>"] = (actions.send_to_qflist + actions.open_qflist), ["<c-f>"] = (actions.send_selected_to_qflist + actions.open_qflist), ["<tab>"] = (actions.toggle_selection + actions.move_selection_better), ["<s-tab>"] = (actions.toggle_selection + actions.move_selection_worse), ["<c-c>"] = actions.delete_buffer, ["<c-u>"] = actions.preview_scrolling_up, ["<c-d>"] = actions.preview_scrolling_down}}, vimgrep_arguments = {"rg", "--color=never", "--no-heading", "--with-filename", "--line-number", "--column", "--smart-case", "--iglob", "!.git", "--hidden"}}, extensions = {["ui-select"] = {themes.get_dropdown({})}, sessions_picker = {sessions_dir = (vim.fn.stdpath("data") .. "/session/")}}, pickers = {find_files = {find_command = {"rg", "--files", "--iglob", "!.git", "--hidden"}}}})
