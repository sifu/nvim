...
local function _5_()
  ...
  cmp.setup({formatting = {format = formatting}, completion = {autocomplete = false}, window = {completion = cmp.config.window.bordered(win_options), documentation = cmp.config.window.bordered(win_options)}, mapping = {["<Up>"] = cmp.mapping.select_prev_item(), ["<Down>"] = cmp.mapping.select_next_item(), ["<C-b>"] = cmp.mapping.scroll_docs(( - 4)), ["<C-f>"] = cmp.mapping.scroll_docs(4), ["<C-Space>"] = cmp.mapping(_6_), ["<C-e>"] = cmp.mapping.close(), ["<S-Tab>"] = cmp.mapping(_8_, {"i", "s"})}, snippet = {expand = _10_}, sources = cmp_srcs})
