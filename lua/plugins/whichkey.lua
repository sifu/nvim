...
local mappings = {
	{ "n", ",S", "<cmd>e /s/tmp/scratchpad.fnl<cr>", "Open Scratchpad" },
	{ "n", ",i", add_to_obsidian_inbox, "Add to Obsidian Inbox" },
	{ "n", ",o", "<cmd>Obsidian<cr>", "Open Obsidian" },
	{ "n", ",s", "<cmd>Obsidian quick_switch<cr>", "Obsidian Quick Switch" },
	...
	{ "n", "<leader>cp", copy_filepath_raw, "Copy filepath raw" },
	...
	{ "n", "<leader>li", find_i18n_key, "Find i18n Translation" },
	{ "n", "<leader>lt", "<cmd>lua vim.lsp.buf.type_definition()<cr>", "TypeScript Type" },
	{ "n", "<leader>lw", "<cmd>TailwindConcealToggle<cr>", "Tailwind Conceal Toggle" },
	{ "n", "<leader>la", "<cmd>lua vim.lsp.buf.code_action()<cr>", "Code Action" },
	{ "n", "<leader>ld", "<cmd>Telescope lsp_definitions<cr>", "Code Definition" },
	{ "n", "<leader>lD", "<c-w>]", "Code Definition Split" },
	{ "n", "<leader>lR", "<cmd>lua vim.lsp.buf.rename()<cr>", "Rename" },
	{ "n", "<leader>lr", "<cmd>Telescope lsp_references<cr>", "References" },
	{ "n", "<leader>lf", ":lua require('user.lsp-unique-references').references()<CR>", "References (unique files)" },
	{ "n", "<leader>ls", "<cmd>Telescope lsp_document_symbols<cr>", "Document Symbols" },
	{ "n", "<leader>lI", "<cmd>lua vim.diagnostic.open_float({ border = 'rounded' })<cr>", "Diagnostic" },
	{ "n", "<leader>lp", "<cmd>lua vim.diagnostic.goto_prev({ border = 'rounded' })<cr>", "Diagnostic" },
	{ "n", "<leader>ln", "<cmd>lua vim.diagnostic.goto_next({ border = 'rounded' })<cr>", "Diagnostic" },
	{ "n", "<leader>lu", ":lua require('user.telescope-routes').routes()<CR>", "Routes" },
	...
	{ "n", "<leader>gC", "<cmd>Telescope git_bcommits<cr>", "Checkout commit" },
	{ "n", "<leader>gt", "<cmd>Tardis<cr>", "Open Tardis" },
	{ "n", "<leader>go", "<cmd>Telescope git_status<cr>", "Open changed file" },
	{ "n", "<leader>gv", "<cmd>lua require 'gitsigns'.preview_hunk()<cr>", "Preview Hunk" },
	...
	{ "n", "<leader>MM", "<cmd>RenderMarkdown toggle<cr>", "Toggle Render-Markdown" },
	...
	{ "n", "\203\153", "<C-W><C-H>", "Window navigation" },
	{ "n", "\194\172", "<C-W><C-L>", "Window navigation" },
	{ "n", "\226\136\134", "<C-W><C-J>", "Window navigation" },
	{ "n", "\203\154", "<C-W><C-K>", "Window navigation" },
	{ "n", "\195\184", "<C-W><C-O>", "Window only" },
	{ "n", "\195\159", "<C-W><C-S>", "Window split" },
	{ "n", "\226\136\154", "<C-W><C-V>", "Window split vertically" },
	{ "n", "\195\167", "<C-W>c", "Window close" },
	...
	{ "n", "<space><space>", ":lua require('telescope.builtin').find_files()<CR>", "Find Files" },
	{ "n", "<space>F", "<cmd>:lua MiniFiles.open(vim.uv.cwd(), true)<CR>", "Explore" },
	{ "n", "<space>d", "<cmd>Telescope oil<CR>", "Explore Directory" },
	{ "n", "<space>g", ":lua require('user.telescope-multigrep').multigrep()<CR>", "Multigrep (with `  `)" },
	{ "n", "<space>w", "<cmd>Telescope grep_string<cr>", "Find Current Word" },
	{ "n", "<space>m", "<cmd>Telescope macroscope<cr>", "Macro History" },
	{ "n", "<space>s", "<cmd>Telescope sessions_picker<cr>", "Sessions" },
	{
		"n",
		"<space>h",
	...
	{ "n", "<space>r", ":lua require('telescope.builtin').resume()<CR>", "Resume last search" },
	{
		"n",
		"<space>e",
...
local function add_mapping(wk, _9_)
	local mode = _9_[1]
	local key = _9_[2]
	local mapping = _9_[3]
	local desc = _9_[4]
...
local function add_group(wk, _10_)
	local prefix = _10_[1]
	local group = _10_[2]
...
local function add_groups_and_mappings()
	...
	core.map(_11_, groups)
	local function _12_(_241)
		return add_mapping(wk, _241)
	end
	return core.map(_12_, mappings)
end
